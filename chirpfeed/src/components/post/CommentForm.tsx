...
export function CommentForm({ postId, onCommentCreated }: CommentFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await createComment(postId, text, imageData);
  ...
  return (
    <form onSubmit={handleSubmit} className="p-4 border-t border-border animate-scale-in">
      <Textarea
        value={text}
        onChange={handleTextChange}
        placeholder="Post your reply"
        className="resize-none border-0 focus-visible:ring-0 text-lg p-0 h-16 bg-transparent"
      />
